...
The lean defaults answer informational questions and stop, delegate only independently reviewable work, and choose a focused verification budget before implementation. Muse handoffs request 150 words; displayed JSON is capped at 3,000 characters by default. These are behavioral instructions plus an output limit—not proof of token savings. Ledgers remain optional. The installer preserves existing global instructions; consolidate old duplicate policies only after reviewing them and obtaining the user's approval.
...
0.3.1 added termination cleanup, stale-measurement detection, and whole-result stdout limits. Version 0.3.2 adds the lean defaults above. Full evidence remains on disk; abrupt SIGKILL protection is not claimed.
...
Expect JSON showing your Python version, Codex home, `muse`/`git` paths, installed version `0.3.2`, and selected options. After install, always use the installed Jev helper path for credential checks, not the package copy: private settings live beside the installed skill. With a custom home, substitute its `skills/jev-context/scripts/context_filter.py`.
...
Expect status `completed` and the correct version `0.3.2`.
